...
//                    append("data").append(OtherConfig.getOsSplitting()).append("ver_token_baru.html").toString());
    ...
    @Override
    public ResponseEntity<Object> save(User user, HttpServletRequest request) {//RANGE 001-010
        if(user==null)
        {
            return new ResponseHandler().generateResponse(
                    ConstantMessageGlobal.ERROR_DATA_INVALID,//message
                    HttpStatus.BAD_REQUEST,//httpstatus
                    null,//object
                    "FV005001",//errorCode Fail Validation modul-code 001 sequence 001 range 001 - 010
                    request
            );
        }
        ...
        {
            strExceptionArr[1] = "save(User user, HttpServletRequest request)  --- LINE 361 \n"+ RequestCapture.allRequest(request);
            LoggingFile.exceptionStringz(strExceptionArr, e, OtherConfig.getFlagLoging());
            LogTable.inputLogRequest(strExceptionArr,e, OtherConfig.getFlagLogTable());
            return new ResponseHandler().generateResponse(
                    ConstantMessageGlobal.ERROR_SAVE_FAILED,//message
                    HttpStatus.INTERNAL_SERVER_ERROR,//httpstatus
                    null,//object
                    "FE005001",//errorCode Fail Error modul-code 001 sequence 001 range 001 - 010
                    request
            );
        }
